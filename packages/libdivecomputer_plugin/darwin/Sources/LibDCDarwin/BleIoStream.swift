...
class BleIoStream: NSObject, CBPeripheralDelegate {
    ...
    private func initialWriteWithoutResponsePreference(for characteristic: CBCharacteristic) -> Bool {
        let properties = characteristic.properties
        // Crest over Nordic UART wants acknowledged writes: NUS RX advertises
        // both write modes, so the generic rule below would pick neither
        // decisively and the +4 bias on writeWithoutResponse would win.
        if BleCharacteristicSelector.isCrestName(peripheral.name),
            characteristic.uuid == BleCharacteristicSelector.nordicUartRxUUID {
            return false
        }
    ...
    private func finalizeCharacteristicSelection() {
        ...
        // The peripheral name is passed so the selector can apply its
        // Crest-gated Nordic UART bias; every other device is unaffected.
        guard let selection = BleCharacteristicSelector.select(
            services: services, peripheralName: peripheral.name) else {
            NativeLogger.w("BleIoStream", category: "BLE",
