...
enum BleCharacteristicSelector {
    ...
    //
    // Crest computers (CR5L) expose a plain Nordic UART Service rather than a
    // vendor service. NUS is a generic Nordic SDK service used by an enormous
    // range of unrelated BLE devices, so it is NOT added to the preferred sets
    // unconditionally: biasing every NUS device towards this pair could pull a
    // future dive computer off a vendor service that scores lower on raw
    // properties. The bias is gated on the peripheral advertising a Crest
    // name, which is what `isCrestName` decides. Widening support to more NUS
    // devices starts by widening that gate.

    /// Nordic UART Service.
    static let nordicUartServiceUUID = CBUUID(
        string: "6E400001-B5A3-F393-E0A9-E50E24DCCA9E")
    /// NUS RX: the client writes commands here.
    static let nordicUartRxUUID = CBUUID(
        string: "6E400002-B5A3-F393-E0A9-E50E24DCCA9E")
    /// NUS TX: the server notifies replies here.
    static let nordicUartTxUUID = CBUUID(
        string: "6E400003-B5A3-F393-E0A9-E50E24DCCA9E")

    /// Whether a peripheral name identifies a Crest computer, the only family
    /// the NUS bias applies to.
    static func isCrestName(_ name: String?) -> Bool {
        (name?.uppercased().hasPrefix("CREST")) ?? false
    }
    ...
    ///
    /// `isCrest` enables the Nordic UART bias; see the NUS section above for
    /// why it is name-gated rather than always on.
    static func writeScore(_ characteristic: Characteristic, isCrest: Bool = false) -> Int? {
        ...
        if isCrest && characteristic.uuid == nordicUartRxUUID { score += 1000 }
        return score
    }

    /// Score a notify candidate, or nil if the characteristic cannot notify.
    ///
    /// `isCrest` enables the Nordic UART bias; see the NUS section above.
    static func notifyScore(_ characteristic: Characteristic, isCrest: Bool = false) -> Int? {
        let properties = characteristic.properties
        ...
        if isCrest && characteristic.uuid == nordicUartTxUUID { score += 1000 }
        return score
    }
    ...
    static func select(services: [Service], peripheralName: String? = nil) -> Selection? {
        let isCrest = isCrestName(peripheralName)
        var best: Selection?
        for (serviceIndex, service) in services.enumerated() {
            ...
            for (index, characteristic) in service.characteristics.enumerated() {
                if let score = writeScore(characteristic, isCrest: isCrest),
                    bestWrite == nil || score > bestWrite!.score {
                    bestWrite = (index, score)
                }
                if let score = notifyScore(characteristic, isCrest: isCrest),
            ...
            if isCrest && service.uuid == nordicUartServiceUUID { serviceScore += 1000 }
