...
// 15. Crest (CR5L) over the Nordic UART Service. NUS is a generic Nordic SDK
// service, so the bias is gated on the peripheral name: with a Crest name the
// NUS pair must win, and with any other name the same services must select
// exactly as they would without the gate.
do {
    let nusService = "6E400001-B5A3-F393-E0A9-E50E24DCCA9E"
    let nusRx = "6E400002-B5A3-F393-E0A9-E50E24DCCA9E"
    let nusTx = "6E400003-B5A3-F393-E0A9-E50E24DCCA9E"
    let vendorWrite = "0000AAA1-0000-1000-8000-00805F9B34FB"
    let vendorNotify = "0000AAA2-0000-1000-8000-00805F9B34FB"
    let services = [
        // A vendor service that outscores NUS on raw properties alone.
        BleCharacteristicSelector.Service(
            uuid: CBUUID(string: "0000AAA0-0000-1000-8000-00805F9B34FB"),
            characteristics: [
                char(vendorWrite, [.write, .writeWithoutResponse]),
                char(vendorNotify, [.notify]),
            ]
        ),
        BleCharacteristicSelector.Service(
            uuid: CBUUID(string: nusService),
            characteristics: [
                char(nusRx, [.write, .writeWithoutResponse]),
                char(nusTx, [.notify]),
            ]
        ),
    ]

    let crest = resolve(services, BleCharacteristicSelector.select(
        services: services, peripheralName: "CREST CR5L"))
    expect(crest?.write == CBUUID(string: nusRx),
           "crest-nus: commands go to NUS RX")
    expect(crest?.notify == CBUUID(string: nusTx),
           "crest-nus: replies arrive on NUS TX")

    let other = resolve(services, BleCharacteristicSelector.select(
        services: services, peripheralName: "Perdix 2"))
    expect(other?.write == CBUUID(string: vendorWrite),
           "crest-nus: a non-Crest name is not pulled onto NUS")

    let unnamed = resolve(services, BleCharacteristicSelector.select(services: services))
    expect(unnamed?.write == CBUUID(string: vendorWrite),
           "crest-nus: an unnamed peripheral keeps the ungated selection")

    expect(BleCharacteristicSelector.isCrestName("crest cr5l"),
           "crest-nus: the name gate is case-insensitive")
    expect(!BleCharacteristicSelector.isCrestName(nil),
           "crest-nus: a nil name is not Crest")
}
