...
If you develop on macOS or iOS without an Apple Developer account,
`flutter run` fails with `No profiles for 'app.submersion' were found`. The
Debug configuration is pinned to the maintainer's signing team. Sign the debug
build ad-hoc instead, as described in
...
Linux, Windows and Android need no signing setup.
