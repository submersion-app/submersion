...
/// Schema v232: trip-scale gas logistics, phase 1 (issue #2325). Two children
/// of trips, trip_cylinders and trip_cylinder_events, and the
/// dive_tanks.trip_cylinder_id link.
void main() {
  /// Pre-v232 dive_tanks: only the columns the rung and its assertions touch.
  const preV232DiveTanks = '''
  ...
  /// the beforeOpen backstops touch, one pre-v232 tank row, and neither new
  ...
  NativeDatabase setupDb({
    ...
    return NativeDatabase.memory(
      setup: (rawDb) {
        ...
        rawDb.execute(preV232DiveTanks);
        rawDb.execute(
  ...
  test(
    'a database stamped v232 without the tables heals in beforeOpen',
    () async {
      // A parallel branch that claimed 232 first carries a device past the
