...
void main() {
  test('v225 is at or below the current schema version and in the ladder', () {
    // Relaxed once v226 (media cloud asset id) landed on top; the newest
    // rung owns the exact assertion.
    expect(AppDatabase.currentSchemaVersion, greaterThanOrEqualTo(225));
    expect(AppDatabase.migrationVersions, contains(225));
  });
  ...
  test(
    'a database stranded before v225 gains the column via beforeOpen',
    () async {
  ...
  test('a v224 database upgrades and gains the column', () async {
    final nativeDb = NativeDatabase.memory(
      setup: (rawDb) {
        rawDb.execute('PRAGMA user_version = 224');
        rawDb.execute('''
          CREATE TABLE dives (
            id TEXT NOT NULL PRIMARY KEY,
            dive_date_time INTEGER NOT NULL,
