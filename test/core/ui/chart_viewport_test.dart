...
void main() {
  group('ChartViewport', () {
    ...
    test('zoomedAt center keeps the center centered', () {
      final vp = ChartViewport.reset.zoomedAt(0.5, 0.5, 2);
      expect(vp.zoom, 2.0);
    ...
    test('zoomedAt top-left pins the top-left corner', () {
      final vp = ChartViewport.reset.zoomedAt(0, 0, 2);
    ...
    test('zoomedAt bottom-right pins the bottom-right corner', () {
      final vp = ChartViewport.reset.zoomedAt(1, 1, 2);
    ...
    test('anchor invariant: the data point under the focal stays fixed', () {
      ...
      for (final c in cases) {
        const start = ChartViewport(zoom: 2, offsetX: 0.25, offsetY: 0.25);
    ...
    test('pannedBy clamps to [0, 1 - 1/zoom]', () {
      const vp = ChartViewport(zoom: 2, offsetX: 0.25, offsetY: 0.25);
    ...
    test('zoom clamps to [minZoom, maxZoom] and is a no-op at the rail', () {
      ...
      final out = ChartViewport.reset.zoomedAt(0.5, 0.5, 0.5);
      expect(out.zoom, 1.0);
      expect(out.offsetX, 0.0);
      // At max zoom, zooming further in is a no-op.
      const atMax = ChartViewport(zoom: 10, offsetX: 0.4, offsetY: 0.4);
    ...
    test('zoom in then out at the same focal round-trips to the origin', () {
      final there = ChartViewport.reset.zoomedAt(0.3, 0.7, 2);
  ...
  group('trackpad scroll maps to a cursor-anchored zoom factor', () {
    ...
    test('scroll down produces a >1 factor (zoom in)', () {
      ...
      final vp = const ChartViewport().zoomedAt(0.5, 0.5, factor);
