...
void main() {
  ...
  test('pathFor puts submersion.keys next to the database file', () {
    expect(
      DatabaseSecuritySidecar.pathFor(p.join('a', 'b', 'submersion.db')),
      p.join('a', 'b', 'submersion.keys'),
    );
  });
  ...
  group('isEncryptedDatabaseFile', () {
    test('false for missing file', () {
      expect(isEncryptedDatabaseFile(p.join(tmp.path, 'nope.db')), false);
    });

    test('false for a plaintext SQLite header', () {
      final f = File(p.join(tmp.path, 'plain.db'));
      f.writeAsBytesSync([
    ...
    test('true for random (encrypted-looking) bytes', () {
      final f = File(p.join(tmp.path, 'enc.db'));
      f.writeAsBytesSync(
    ...
    test('false for a short/empty file', () {
      final f = File(p.join(tmp.path, 'tiny.db'))..writeAsBytesSync([1, 2, 3]);
      expect(isEncryptedDatabaseFile(f.path), false);
    });
  });
}
