...
void main() {
  ...
  group('with the statistics filter on', () {
    ...
    setUp(() {
      container.read(insightsFilterProvider.notifier).state = DiveFilterState(
    ...
    test('a gear link on an existing dive reaches the filtered set', () async {
      ...
      final sub = container.listen(insightsFilteredDiveIdsProvider, (_, _) {});
      addTearDown(sub.close);
      expect(await container.read(insightsFilteredDiveIdsProvider.future), {
        'b1',
        'b2',
      });
      ...
      for (var i = 0; i < 100; i++) {
        ...
        ids = await container.read(insightsFilteredDiveIdsProvider.future);
