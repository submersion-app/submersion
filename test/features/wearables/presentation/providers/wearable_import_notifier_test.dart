...
@GenerateMocks([DiveRepository, WearableImportService])
import 'wearable_import_notifier_test.mocks.dart';

void main() {
  group('WearableImportNotifier', () {
    late MockDiveRepository mockRepository;
    late MockWearableImportService mockService;
    late WearableImportNotifier notifier;
    const matcher = DiveMatcher();
    const converter = WearableDiveConverter();

    setUp(() {
      mockRepository = MockDiveRepository();
      mockService = MockWearableImportService();
      notifier = WearableImportNotifier(mockService);
    });

    WearableDive createWearableDive({
    ...
    group('checkForDuplicates', () {
      test('marks dives with matching wearableId as alreadyImported', () async {
        ...
        expect(
          ...
          equals(WearableDiveMatchStatus.alreadyImported),
        );
        expect(results['new-dive-id'], equals(WearableDiveMatchStatus.none));
      });

      test('marks fuzzy matches as probable or possible', () async {
        final wDive = createWearableDive(
        ...
        expect(
          ...
          equals(WearableDiveMatchStatus.probable),
        );
      });
      ...
      test('marks non-matching dives as none', () async {
        ...
        expect(
          ...
          equals(WearableDiveMatchStatus.none),
        );
      });
    ...
    group('performImport', () {
      test('imports new dives and skips duplicates', () async {
        final newDive = createWearableDive(
          sourceId: 'new-dive',
          startTime: DateTime(2024, 6, 15, 10, 0),
        );
        final dupDive = createWearableDive(
          sourceId: 'dup-dive',
          startTime: DateTime(2024, 6, 16, 10, 0),
        );
        final importedDive = createWearableDive(
      ...
      test('imports possible duplicates (user chose them)', () async {
        final possibleDup = createWearableDive(
        ...
        expect(
          ...
          equals(WearableDiveMatchStatus.possible),
        );
