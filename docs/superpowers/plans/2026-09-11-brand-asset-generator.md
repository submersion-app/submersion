...
**Architecture:** A new `scripts/brand/` folder holds the moved icon generator, a pure-data preset table, a Pillow-free layout module, a transparent-mark module built from the icon's own shape primitives, and a CLI renderer that draws with Pillow and bundled Inter Display fonts. Output goes to a gitignored `scripts/brand/out/`.
...
- No generated PNG is committed. `scripts/brand/out/` is gitignored.
