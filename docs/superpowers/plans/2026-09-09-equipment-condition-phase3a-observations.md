...
### app_es.arb, app_fr.arb, app_it.arb, app_nl.arb, app_pt.arb, app_hu.arb, app_ar.arb, app_he.arb, app_zh.arb

Translate the same three blocks into each language at execution time, keeping every key, placeholder and ICU shape identical to English, and record each block in this appendix before committing Task 6, Task 10 and Task 12 respectively, so the plan stays the single source for the strings that shipped. Glossary to keep consistent across the ten languages: "check-in" (a recorded gear check), "bench" (a check made off a dive), "free flow", "scrubber breakthrough", "dropout" (a transmitter losing contact).
