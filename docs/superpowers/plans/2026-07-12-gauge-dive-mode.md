...
>
> IMPLEMENTATION NOTE (as-built): `importProfile` lives in
> `dive_log/data/repositories/dive_computer_repository_impl.dart` (NOT
> `dive_repository_impl.dart`). `diveMode` is written ONLY on the new-dive INSERT
> companion, not the `!isNewDive` update path, to avoid interfering with
> multi-computer consolidation's scalar-column semantics (a re-import/match keeps
> the dive's existing mode). Both `_importNewDive` and `_updateExistingDive` pass
> `diveMode: dive.diveMode`. Changing `importProfile`'s signature invalidates the
> Mockito mock — run `dart run build_runner build --delete-conflicting-outputs`
> (the `.mocks.dart` files are gitignored and regenerated by CI). Verified with a
> repository persistence test asserting the stored `dive_mode`.
