...
| Profile events / markers | [ ] | Medium | `setpointChange`, `bookmark`, `safetyStopStart`, `decoStopStart`, `decoViolation` (from `ceiling` / `violation`), `ascentRateWarning` (from `ascent`), `ppO2High` and `ppO2Low` (from `po2`, threshold-split at 1.4 / 0.18 bar) are now persisted via `DiveProfileEvents` (Slice C + C.2). DC-internal alarms (`low battery`, `heading`, `rbt`) remain intentionally unmapped. UDDF parity is Slice C.3. |
