...
class GearFeature extends Equatable {
  ...
  factory GearFeature.fromEquipment({
    ...
    if (buoyancyKg != null) {
      ...
      prior = _typePrior(type, name, size);
  ...
  static double _typePrior(EquipmentType type, String name, String? size) {
    switch (type) {
      case EquipmentType.wetsuit:
        final thickness =
            _parseThicknessMm(name) ?? _parseThicknessMm(size ?? '');
        if (thickness != null) return thickness.clamp(0.0, 8.0);
        return 4.0;
  ...
  static double? _parseThicknessMm(String text) {
    final match = _thicknessPattern.firstMatch(text);
