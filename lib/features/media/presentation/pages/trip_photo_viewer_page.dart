...
///
/// Displays all photos in a trip chronologically with swipe navigation.
/// Dive context overlays (site name, depth, temp, elapsed time) update
/// automatically when swiping between photos from different dives.
class TripPhotoViewerPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<TripPhotoViewerPage> createState() =>
      _TripPhotoViewerPageState();
}
...
class _TripPhotoViewerPageState extends ConsumerState<TripPhotoViewerPage> {
  ...
  /// Find which dive a media item belongs to.
  Dive? _findDiveForMedia(
    MediaItem item,
    Map<Dive, List<MediaItem>> mediaByDive,
  ) {
  ...
  Widget build(BuildContext context) {
    // Watch flat list for gallery navigation
    final flatMediaAsync = ref.watch(
      flatMediaListForTripProvider(widget.tripId),
    );
    // Watch grouped media for dive context lookup
    final groupedMediaAsync = ref.watch(mediaForTripProvider(widget.tripId));
    ...
    return Scaffold(
      ...
      body: flatMediaAsync.when(
        data: (mediaList) {
          ...
          // Get dive context for current photo
          final mediaByDive = groupedMediaAsync.valueOrNull ?? {};
          final currentDive = _findDiveForMedia(currentItem, mediaByDive);
          final diveProfile = currentDive?.profile ?? [];
            ...
            child: Stack(
              children: [
                ...
                if (_showOverlay) ...[
                  ...
                  // Bottom metadata with dive context
                  _BottomMetadataOverlay(
                    item: currentItem,
                    settings: settings,
                    siteName: currentDive?.site?.name,
                  ),
                ],
              ],
            ),
