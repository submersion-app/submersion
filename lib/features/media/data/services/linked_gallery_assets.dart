...
class LinkedGalleryAssets {
  ...
  ///
  /// Rows the resolver cannot pin to one asset then go through the burst
  /// rule: see [_claimBursts].
  Future<List<AssetInfo>> withoutLinked({
    ...
    for (final row in notYetMatched) {
      ...
      if (localId == null) {
        unresolved.add(row);
      } else if (candidateIds.contains(localId)) {
        claimed.add(localId);
      }
      // Otherwise the row is some asset outside this candidate set.
    }

    final remaining = candidates.where((c) => !claimed.contains(c.id)).toList();
    claimed.addAll(_claimBursts(remaining, unresolved));
    return candidates.where((c) => !claimed.contains(c.id)).toList();
  }

  /// The burst rule, for rows the resolver declined.
  ///
  ...
  /// when at least as many unresolved rows match a frame (second + size) as
  ...
  /// [AssetResolutionService.matchesTimestampAndDimensions]) land on two
  /// different frames is counted towards neither.
  static Set<String> _claimBursts(
    List<AssetInfo> remaining,
    List<MediaItem> unresolved,
  ) {
    if (remaining.isEmpty || unresolved.isEmpty) return const {};
    ...
    for (final row in unresolved) {
      // Every candidate in a frame shares its second and size, so any one of
      // them stands for the frame.
      final frames = byFrame.entries
          .where(
            (e) => AssetResolutionService.matchesTimestampAndDimensions(
              row,
              e.value.first,
  ...
  /// A candidate's frame: capture second and pixel size. photo_manager
  /// reports whole seconds, so the division drops nothing.
