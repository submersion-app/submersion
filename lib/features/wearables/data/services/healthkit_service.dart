...
/// HealthKit implementation of WearableImportService for Apple platforms.
///
...
class HealthKitService implements WearableImportService {
  ...
  /// Convert a HealthKit workout to a WearableDive entity.
  Future<WearableDive?> _workoutToDive(HealthDataPoint workoutPoint) async {
  ...
  Future<List<WearableProfileSample>> _fetchWorkoutSamples(
    DateTime startTime,
    DateTime endTime,
  ) async {
    try {
      ...
      final samples = <WearableProfileSample>[];
  ...
  double _calculateMaxDepth(List<WearableProfileSample> samples) {
    if (samples.isEmpty) return 0.0;
    return samples.map((s) => s.depth).reduce((a, b) => a > b ? a : b);
  }

  /// Calculate average depth from samples.
  double _calculateAvgDepth(List<WearableProfileSample> samples) {
    if (samples.isEmpty) return 0.0;
  ...
  double? _calculateAvgHeartRate(List<WearableProfileSample> samples) {
