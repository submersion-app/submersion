...
/// Supplies the Material date pickers with a compact (numeric) date format of
/// our choosing while leaving every other string in the app's own language.
///
/// The pickers offer no hook for the manual-entry format on its own. The only
/// lever `showDatePicker` exposes is its `locale` argument, and Flutter
/// implements that as a whole-tree `Localizations.override`, so borrowing
/// `de` for its dd.MM.yyyy input format also translated the dialog title,
/// buttons, and month names into German (#1510).
///
/// Inserting this delegate instead overrides only [MaterialLocalizations],
/// and only its three compact-date members. `Localizations.override` merges
/// our delegate ahead of the app's, and `Localizations` keeps the first
/// delegate registered for a type, so the rest of the dialog resolves through
/// the app's delegates at the app's locale.
class CompactDateLocalizationsDelegate
    extends LocalizationsDelegate<MaterialLocalizations> {
  const CompactDateLocalizationsDelegate({
    required this.base,
    required this.pattern,
  });

  /// The app-language localizations every non-date member forwards to.
  final MaterialLocalizations base;

  /// An `intl` date pattern such as `dd.MM.yyyy` describing how the diver
  /// wants to type and read dates.
  ...
  Future<MaterialLocalizations> load(Locale locale) {
    ...
    return SynchronousFuture<MaterialLocalizations>(
      _CompactDateMaterialLocalizations(
        base: base,
        format: _dateFormat(pattern, localeName),
        helpText: pattern.toLowerCase(),
      ),
    );
  }

  @override
  bool shouldReload(CompactDateLocalizationsDelegate old) =>
      old.base != base || old.pattern != pattern;
}

/// Builds the pattern against [localeName] so locale-specific digits survive,
/// falling back the way `GlobalMaterialLocalizations` does when `intl` has no
/// data for the exact locale.
intl.DateFormat _dateFormat(String pattern, String localeName) {
...
/// Forwards all of [MaterialLocalizations] to [_base] except the members the
/// date pickers use for manual entry.
///
/// The forwarding is exhaustive by construction: `implements` makes the
/// analyzer fail the build if a future Flutter release adds a member, which is
/// louder than a wrapper that silently answers in the wrong language.
class _CompactDateMaterialLocalizations implements MaterialLocalizations {
  const _CompactDateMaterialLocalizations({
    required MaterialLocalizations base,
  ...
  @override
  String formatCompactDate(DateTime date) => _format.format(date);
  ...
  @override
  String get dateHelpText => _helpText;
  ...
  // The reorder hints below moved to WidgetsLocalizations, but they stay
  // abstract on MaterialLocalizations, so the wrapper still has to answer
  // them. Forwarding is the only correct answer while they exist.
  @override
